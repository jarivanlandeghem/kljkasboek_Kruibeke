describe('Kasjes page - budgetbeheer', () => {
  ...
  it('Admin kan budget aanpassen (PUT wordt gestuurd)', () => {
    ...
    cy.intercept('GET', 'http://localhost:3000/api/users/me', { statusCode: 200, body: { userid: 1, voornaam: 'Admin', roles: ['ADMIN'] } }).as('me');

    const kasjes = [ { kasjeID: 10, groep: '-12', bedrag: 200 } ];
    cy.intercept('GET', 'http://localhost:3000/api/transacties', { statusCode: 200, body: [] }).as('getTx');
    cy.intercept('GET', 'http://localhost:3000/api/kasjes', { statusCode: 200, body: kasjes }).as('getKasjes');
    ...
    cy.contains('-12').closest('.rounded-2xl').within(() => {
      ...
      cy.intercept('PUT', 'http://localhost:3000/api/kasjes/10', (req) => {
  ...
  it('Normale user ziet geen edit icoon', () => {
    // normal user
    cy.window().then((win) => win.localStorage.setItem('jwtToken', 'fake-jwt-token'));
    cy.intercept('GET', 'http://localhost:3000/api/users/me', { statusCode: 200, body: { userid: 2, voornaam: 'User', roles: ['user'] } }).as('meUser');

    const kasjes = [ { kasjeID: 11, groep: '-8', bedrag: 150 } ];
    cy.intercept('GET', 'http://localhost:3000/api/transacties', { statusCode: 200, body: [] }).as('getTx2');
    cy.intercept('GET', 'http://localhost:3000/api/kasjes', { statusCode: 200, body: kasjes }).as('getKasjes2');
